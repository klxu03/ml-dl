...
def explore_mlp_activations(dev_d: Dict[str, List[Union[str, int]]],
    ...
    for act, actn, in zip(activations, activation_names):
        train_config = EasyDict({
            ...
            'save_path': f'model_activations_{actn}.pth',  # path where to save the model
            'embeddings': embedding_type,
            'num_classes': num_classes,
        ...
        all_emb_epoch_dev_accs.append(epoch_dev_accs)
        all_emb_epoch_dev_losses.append(epoch_dev_loss)
        visualize_epochs(epoch_train_losses, epoch_dev_loss, f"activation_structure_{actn}_loss.png")

    visualize_configs(all_emb_epoch_dev_accs, activation_names, "Accuracy", "./all_activation_structures_acc.png")
    visualize_configs(all_emb_epoch_dev_losses, activation_names, "Loss", "./all_activation_structures_loss.png")
    # your code ends here
...
def explore_mlp_learning_rates(dev_d: Dict[str, List[Union[str, int]]],
    ...
    for lr, lr_n, in zip(lrs, lrs_names):
        train_config = EasyDict({
            'batch_size': batch_size,  # we use batching for training
            'lr': lr,
            'num_epochs': num_epochs,  # the total number of times all the training data is iterated over
            'hidden_dims': hidden_dims,  # the number of neurons in each hidden layer
            'save_path': f'learning_rate_{lr_n}.pth',  # path where to save the model
            'embeddings': embedding_type,
            'num_classes': num_classes,
            'activation': activation,  # non-linear activation function
        })
    ...
    visualize_configs(all_emb_epoch_dev_accs, lrs_names, "Accuracy", "./all_lr_structures_acc.png")
    visualize_configs(all_emb_epoch_dev_losses, lrs_names, "Loss", "./all_lr_structures_loss.png")
    # your code ends here
