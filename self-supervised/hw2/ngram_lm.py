...
def create_ngrams(data, n, splitter, tokenizer):
    ...
    for paragraph in tqdm(data['text']):
        ...
        for sentence in splitter.split(paragraph):
            ...
            # TOD: tokenize the words in the sentence
            ...
            for idx, _ in enumerate(tokens):
                if idx + n >= len(tokens):
                    ...
                    break
                # TOD: count the occurrence of the n-gram, where
    ...
    for context, next_words in sorted_next_word_candidates.items():
        ...
        for nw in next_words:
            # TOD: compute the estimated probability of the next word given the context
            # hint: use the counters 'ngrams' and 'ngram_context' you have created above
            scores.append(ngrams[tuple(context + (nw,))] / ngram_context[context])
            # Your code ends here
...
def plot_next_word_prob(word_scores, word_candidates, context, top=10, save_path=None):
    # TOD: plot the top 10 next word probabilities after the given context
    ...
    fig, ax = plt.subplots()
    top_indices = np.argsort(word_scores[context])[::-1][:top]
    ax.barh([word_candidates[context][i] for i in top_indices], [word_scores[context][i] for i in top_indices])
    print(f"top_indices: {top_indices} \n word_candidates: {[word_candidates[context][i] for i in top_indices]} \n word_scores: {[word_scores[context][i] for i in top_indices]}")
    ...
    for i in ax.patches:
        ...
        # Add Plot Title
    ax.set_title(f'Top 10 next word probabilities after context: {context}',
                loc ='left', )
...
def run_ngram():
    ...
    # TOD: paste the generated plots to the pdf
    context1 = ("move", "to")
    context2 = ("the", "news")
    ...
    # TOD: paste the generated completion of these two prefixes to the pdf
